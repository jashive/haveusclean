...
async function rest(session, resource, { method = "GET", body, prefer = "return=representation", allowFailure = false } = {}) {
  ...
  if (!res.ok && !allowFailure) fail(`${session.role} ${method} ${resource.split("?")[0]} failed HTTP ${res.status}: ${typeof data === "string" ? data.slice(0, 240) : JSON.stringify(data).slice(0, 240)}`);
...
async function run() {
  ...
  try {
    ...
    const unauthorizedServiceRequest = {
      organization_id: scope.organization_id, business_unit_id: scope.business_unit_id,
      service_category: "residential", lifecycle_status: "qualified", intake_channel: "oat_negative",
      title: `${runId}-UNAUTHORIZED`, metadata: { acceptance: true, runId }, requirements: {},
    };
    ...
    await expectDenied("owner worker impersonation", insert(sessions.owner, "completion_evidence", {
      organization_id: scope.organization_id, business_unit_id: scope.business_unit_id,
      operational_job_id: "00000000-0000-0000-0000-000000000001", work_order_id: "00000000-0000-0000-0000-000000000001",
      evidence_type: "note", evidence_payload: { negative: true }, captured_at: new Date().toISOString(), captured_by_app_user_id: sessions.owner.appUser.id,
    }, { allowFailure: true }));
    ...
    const pricing = await insert(office, "pricing_snapshot", { organization_id: org, business_unit_id: buId, opportunity_id: opp.id, estimate_id: est.id, currency_code: "CAD", tax_name: "HST", tax_rate: 0.13, subtotal_amount: 300, discount_amount: 0, tax_amount: 39, total_amount: 339, calculator_version: "TEST-W6-OAT-1", configuration_snapshot: { acceptance: true }, labor_economics: { teamSize: 1, jobHours: 3, partnerPayTotal: 100, profit: 200 }, calculation_inputs: {}, calculation_outputs: { subtotal: 300, tax: 39, total: 339 }, raw_calculation_snapshot: { runId }, frozen_at: new Date().toISOString(), metadata: meta, created_by_app_user_id: office.appUser.id });
    ...
    const location = await insert(office, "service_location", { customer_id: customer.id, jurisdiction_id: jur, label: `${runId} Location`, address_line1: "1 Synthetic Acceptance Way", city: "Test", subdivision: "ON", postal_code: "A1A1A1", country_code: "CA", access_notes: "Synthetic only", metadata: meta });
    ...
    const duplicateJob = insert(office, "operational_job", { ...job, id: undefined }, { allowFailure: true });
    await expectDuplicateRejected("duplicate job handoff", duplicateJob);
    const start = new Date(Date.now() + 3600_000).toISOString();
    const end = new Date(Date.now() + 3 * 3600_000).toISOString();
    const window = await insert(office, "schedule_window", { organization_id: org, business_unit_id: buId, jurisdiction_id: jur, operational_job_id: job.id, scheduled_start: start, scheduled_end: end, timezone: "America/Toronto", status: "planned", scheduling_notes: runId, metadata: meta, created_by_app_user_id: office.appUser.id, updated_by_app_user_id: office.appUser.id });
    ...
    await patch(sessions.worker, "work_order", workOrder.id, { work_order_status: "in_progress", started_at: new Date().toISOString(), updated_by_app_user_id: sessions.worker.appUser.id });
    ...
    await patch(sessions.worker, "work_order", workOrder.id, { work_order_status: "service_complete", service_completed_at: new Date().toISOString(), updated_by_app_user_id: sessions.worker.appUser.id });
    ...
    job = await patch(sessions.qa, "operational_job", job.id, { operational_status: "corrective_action_required", updated_by_app_user_id: sessions.qa.appUser.id });
    await insert(sessions.worker, "completion_evidence", { organization_id: org, business_unit_id: buId, operational_job_id: job.id, work_order_id: workOrder.id, worker_assignment_id: assignment.id, evidence_type: "note", evidence_payload: { note: `${runId} rework completion evidence`, corrective_action_id: corrective.id }, captured_at: new Date().toISOString(), captured_by_worker_id: workerRow.id, captured_by_app_user_id: sessions.worker.appUser.id, metadata: meta });
    corrective = await patch(sessions.worker, "corrective_action", corrective.id, { action_status: "resolved", resolution_payload: { runId, resolved_by_worker: true }, resolved_at: new Date().toISOString(), updated_by_app_user_id: sessions.worker.appUser.id });
    ...
    await patch(sessions.qa, "work_order", workOrder.id, { work_order_status: "qa_complete", updated_by_app_user_id: sessions.qa.appUser.id });
    job = await patch(sessions.qa, "operational_job", job.id, { operational_status: "qa_passed", updated_by_app_user_id: sessions.qa.appUser.id });
    ...
    const paymentPayload = { organization_id: org, business_unit_id: buId, invoice_request_id: invoice.id, provider: "preview_test", provider_event_id: paymentEvent, provider_event_type: "payment.settled", provider_reference_id: runId, currency_code: "CAD", amount_observed: 339, payment_status: "verified", event_payload_snapshot: { runId }, observed_at: new Date().toISOString(), is_test_provider: true, metadata: meta, created_by_app_user_id: office.appUser.id };
    const payment = await insert(office, "payment_observation", paymentPayload);
    await expectDuplicateRejected("duplicate provider payment event", insert(office, "payment_observation", paymentPayload, { allowFailure: true }));
    const comp = await insert(office, "contractor_compensation_version", { organization_id: org, business_unit_id: buId, worker_id: workerRow.id, service_family: "residential", version: `${runId}-COMP`, compensation_method: "flat_amount", currency_code: "CAD", rate_value: 100, effective_from: new Date(Date.now() - 86400_000).toISOString(), compensation_status: "active", approved_by_app_user_id: sessions.owner.appUser.id, approved_at: new Date().toISOString(), governance_reference_snapshot: { runId }, metadata: meta, created_by_app_user_id: office.appUser.id });
    let payable = await insert(office, "contractor_payable", { organization_id: org, business_unit_id: buId, worker_id: workerRow.id, worker_assignment_id: assignment.id, operational_job_id: job.id, work_order_id: workOrder.id, contractor_compensation_version_id: comp.id, compensation_method: "flat_amount", currency_code: "CAD", basis_value: 0, computed_amount: 100, payable_status: "pending", eligibility_assessment: { assignment_completed: true, qa_passed: true, runId }, eligibility_passed: true, metadata: meta, created_by_app_user_id: office.appUser.id });
    await expectDuplicateRejected("duplicate contractor payable", insert(office, "contractor_payable", { ...payable, id: undefined }, { allowFailure: true }));
    payable = await patch(sessions.owner, "contractor_payable", payable.id, { payable_status: "approved", approved_by_app_user_id: sessions.owner.appUser.id, approved_at: new Date().toISOString() });
    const profitability = await insert(office, "job_profitability_snapshot", { organization_id: org, business_unit_id: buId, operational_job_id: job.id, invoice_request_id: invoice.id, currency_code: "CAD", recognized_revenue_amount: 300, tax_amount: 39, direct_labor_cost: 100, other_direct_cost: 0, gross_contribution: 200, gross_margin_percent: 66.6667, source_lineage: { invoice_request_id: invoice.id, contractor_payable_id: payable.id, payment_observation_id: payment.id, runId }, snapshot_taken_at: new Date().toISOString(), metadata: meta, created_by_app_user_id: office.appUser.id });
    ...
    await expectDenied("qa finance mutation", insert(sessions.qa, "payment_observation", { ...paymentPayload, provider_event_id: `${paymentEvent}-QA` }, { allowFailure: true }));
    report.phases.finance = "PASS";

    const concurrentEvent = `${runId}-CONCURRENT`;
    const concurrentPayload = { ...paymentPayload, provider_event_id: concurrentEvent, provider_event_type: "payment.concurrent", metadata: { ...meta, concurrency: true } };
    const concurrent = await Promise.all([
      insert(office, "payment_observation", concurrentPayload, { allowFailure: true }),
      insert(office, "payment_observation", concurrentPayload, { allowFailure: true }),
    ]);
    const successCount = concurrent.filter((x) => x.ok).length;
    assert(successCount === 1, `concurrent duplicate payment expected exactly one success, got ${successCount}`, "HIGH");
    const concurrentRows = await select(office, "payment_observation", `provider=eq.preview_test&provider_event_id=eq.${encodeURIComponent(concurrentEvent)}&select=id`);
    assert(concurrentRows.length === 1, `concurrent duplicate payment persisted ${concurrentRows.length} rows`, "HIGH");
    report.phases.idempotencyConcurrency = "PASS";

    const kpiDefs = await select(sessions.owner, "kpi_definition", `active=eq.true&select=id,code,version&limit=1`);
    ...
    const kpi = await insert(sessions.owner, "kpi_snapshot", { kpi_definition_id: kpiDefs[0].id, kpi_code: kpiDefs[0].code, definition_version: kpiDefs[0].version ?? "1", organization_id: org, business_unit_id: buId, jurisdiction_id: jur, period_type: "DAILY", period_start: periodStart, period_end: periodEnd, timezone: "America/Toronto", numeric_value: 1, numerator: 1, denominator: 1, source_lineage: { operational_job_id: job.id, profitability_snapshot_id: profitability.id, runId }, source_freshness_at: new Date().toISOString(), captured_at: new Date().toISOString(), captured_by_app_user_id: sessions.owner.appUser.id });
    const review = await insert(sessions.owner, "management_review", { organization_id: org, business_unit_id: buId, period_type: "DAILY", period_start: periodStart, period_end: periodEnd, timezone: "America/Toronto", review_status: "in_review", summary: `${runId} controlled pilot OAT management review`, exceptions: [], decisions: [{ decision: "OAT evidence captured" }], actions: [], kpi_snapshot_manifest: [{ id: kpi.id, code: kpi.kpi_code }], owner_app_user_id: sessions.owner.appUser.id, review_version: 1, opened_at: new Date().toISOString(), waiver_recorded: false, created_by_app_user_id: sessions.owner.appUser.id });
    assert(review?.id, "management review insert failed");
    await expectDenied("worker management review mutation", insert(sessions.worker, "management_review", { organization_id: org, business_unit_id: buId, period_type: "DAILY", period_start: periodStart, period_end: periodEnd, timezone: "UTC", review_status: "draft" }, { allowFailure: true }));
    report.phases.kpiManagement = "PASS";

    const finalQa = await select(sessions.owner, "qa_inspection", `operational_job_id=eq.${job.id}&select=id,inspection_status,inspection_type,findings`);
    const finalInvoices = await select(sessions.owner, "invoice_request", `operational_job_id=eq.${job.id}&select=id,request_status,total_amount`);
    const finalPayables = await select(sessions.owner, "contractor_payable", `operational_job_id=eq.${job.id}&select=id,payable_status,computed_amount`);
    const finalProfit = await select(sessions.owner, "job_profitability_snapshot", `operational_job_id=eq.${job.id}&select=id,gross_contribution,gross_margin_percent`);
    ...
    report.status = "PASS";
    report.evidence = { operationalJobId: job.id, qaFailId: qaFail.id, qaPassId: qaPass.id, invoiceRequestId: invoice.id, contractorPayableId: payable.id, profitabilitySnapshotId: profitability.id, managementReviewId: review.id };
