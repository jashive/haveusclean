...
async function run() {
  ...
  try {
    ...
    assert(jurisdiction.currency_code, "acceptance jurisdiction currency missing");
    assert(jurisdiction.country_code, "acceptance jurisdiction country missing");
    assert(jurisdiction.timezone, "acceptance jurisdiction timezone missing");
    ...
    job = await patch(sessions.owner, "operational_job", job.id, { operational_status: "qa_pending", updated_by_app_user_id: sessions.owner.appUser.id });
    ...
    job = await patch(sessions.owner, "operational_job", job.id, { operational_status: "qa_passed", updated_by_app_user_id: sessions.owner.appUser.id });
    ...
    const profitability = await insert(office, "job_profitability_snapshot", { organization_id: org, business_unit_id: buId, operational_job_id: job.id, invoice_request_id: invoice.id, currency_code: currencyCode, recognized_revenue_amount: subtotalAmount, tax_amount: taxAmount, direct_labor_cost: 100, other_direct_cost: 0, gross_contribution: 200, gross_margin_percent: 66.6667, source_lineage: { invoice_request_id: invoice.id, contractor_payable_id: payable.id, payment_observation_id: payment.id, runId }, snapshot_taken_at: new Date().toISOString(), metadata: meta, created_by_app_user_id: office.appUser.id });
    ...
    const kpi = await insert(sessions.owner, "kpi_snapshot", { kpi_definition_id: kpiDefs[0].id, kpi_code: kpiDefs[0].code, definition_version: kpiDefs[0].version ?? "1", organization_id: org, business_unit_id: buId, jurisdiction_id: jur, period_type: "DAILY", period_start: periodStart, period_end: periodEnd, timezone, numeric_value: 1, numerator: 1, denominator: 1, source_lineage: { operational_job_id: job.id, profitability_snapshot_id: profitability.id, runId }, source_freshness_at: new Date().toISOString(), captured_at: new Date().toISOString(), captured_by_app_user_id: sessions.owner.appUser.id });
    const review = await insert(sessions.owner, "management_review", { organization_id: org, business_unit_id: buId, period_type: "DAILY", period_start: periodStart, period_end: periodEnd, timezone, review_status: "in_review", summary: `${runId} controlled pilot OAT management review`, exceptions: [], decisions: [{ decision: "OAT evidence captured" }], actions: [], kpi_snapshot_manifest: [{ id: kpi.id, code: kpi.kpi_code }], owner_app_user_id: sessions.owner.appUser.id, review_version: 1, opened_at: new Date().toISOString(), waiver_recorded: false, created_by_app_user_id: sessions.owner.appUser.id });
