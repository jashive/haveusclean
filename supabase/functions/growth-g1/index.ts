...
function normalizeContact(raw: Record<string, any>, scope: Record<string, string>) {
  const email = normalizeEmail(raw.email);
  const phone = normalizePhone(raw.phone);
  const linkedinUrl = normalizeLinkedIn(raw.linkedin_url || raw.linkedin);
  if (!email && !phone && !linkedinUrl) {
    fail(400, "Contact candidate requires a valid email, 10-digit North American phone, or LinkedIn URL.", "GROWTH_CONTACT_INVALID");
  }
  const verification = text(raw.verification_status).toLowerCase();
  return {
    ...raw,
    ...scope,
    ...
    contact_source: text(raw.contact_source || raw.source_label) || null,
    source_url: text(raw.source_url) || null,
    verification_status: ["unverified", "partially_verified", "verified", "invalid"].includes(verification) ? verification : "unverified",
    metadata: {
      ...(raw.metadata || {}),
...
async function assertTargetScope(organizationId: string, businessUnitId: string, target: {
  prospectId?: string;
  duplicateReviewId?: string;
  contactCandidateId?: string;
}) {
  await rpc("growth_g1_assert_target_scope", {
    p_organization_id: organizationId,
    p_business_unit_id: businessUnitId,
    p_prospect_id: target.prospectId || null,
    p_duplicate_review_id: target.duplicateReviewId || null,
    p_contact_candidate_id: target.contactCandidateId || null,
  });
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const duplicateReviewId = String(body.duplicate_review_id || "");
    const contactCandidateId = String(body.contact_candidate_id || "");
    ...
    if (action === "review_duplicate") {
      if (!duplicateReviewId) fail(400, "duplicate_review_id is required.", "GROWTH_DUPLICATE_REVIEW_REQUIRED");
      await assertTargetScope(organizationId, businessUnitId, { duplicateReviewId });
    } else if (action === "review_contact") {
      if (!contactCandidateId) fail(400, "contact_candidate_id is required.", "GROWTH_CONTACT_REVIEW_REQUIRED");
      await assertTargetScope(organizationId, businessUnitId, { contactCandidateId });
    } else {
      await assertTargetScope(organizationId, businessUnitId, { prospectId });
    }
    ...
    if (action === "add_contact_candidate") {
      const contact = normalizeContact(body.contact || {}, {
        ...
        jurisdiction_id: String(authz.businessUnit.jurisdiction_id),
      });
    ...
    if (action === "review_duplicate") {
      const result = await rpc("growth_g1_review_duplicate", {
        p_duplicate_review_id: duplicateReviewId,
        p_organization_id: organizationId,
        p_decision: String(body.decision || ""),
        p_reviewer_app_user_id: authz.appUser.id,
        p_notes: body.notes || null,
    ...
    if (action === "review_contact") {
      const result = await rpc("growth_g1_review_contact", {
        p_contact_candidate_id: contactCandidateId,
        p_organization_id: organizationId,
        p_decision: String(body.decision || ""),
        p_reviewer_app_user_id: authz.appUser.id,
        p_notes: body.notes || null,
