...
export function publicBookingServerConfig() {
...
async function rest(path, config) {
  const response = await fetch(`${config.url}/rest/v1/${path}`, {
    headers: { apikey: config.secret, Authorization: `Bearer ${config.secret}`, Accept: 'application/json' },
  });
...
async function loadMarketContext(businessUnitCode, config) {
  const units = await rest(`business_unit?select=id,organization_id,jurisdiction_id,code,name,status&code=eq.${encodeURIComponent(businessUnitCode)}&status=eq.active&limit=2`, config);
  ...
  const jurisdictions = await rest(`jurisdiction?select=id,code,country_code,subdivision_code,currency_code,timezone,tax_label,default_tax_rate&id=eq.${encodeURIComponent(unit.jurisdiction_id)}&limit=1`, config);
...
export async function calculatePublicBookingQuote(body, config = publicBookingServerConfig()) {
  ...
  const addons = removeBundledAddonsForPackage({ packageKey, addons: requestedAddons, businessUnitCode, configurationVersion });
  const approvedSelections = getDefaultApprovedSelections(configurationVersion, { condition, frequency, sqftBand: null, sqft });
  const base = computeGovernedResidentialQuote({ configurationVersion, dwellingType, beds, baths, packageKey, condition, frequency, addons, approvedSelections });
