...
const FACILITY_OPTIONS = [
  ...
  ['retail', 'Retail'],
  ['industrial', 'Industrial / Warehouse Office'],
];
...
const styles = {
  ...
  success: { color: '#A7F3D0', marginTop: 12, whiteSpace: 'pre-wrap', fontSize: 13 },
  modeWrap: { display: 'grid', gridTemplateColumns: 'repeat(2,minmax(0,1fr))', gap: 10, marginBottom: 20 },
  mode: { border: '1px solid #334155', borderRadius: 12, padding: '13px 12px', background: '#0B1220', color: '#CBD5E1', fontWeight: 800, cursor: 'pointer' },
  modeActive: { borderColor: '#22D3EE', color: '#ECFEFF', boxShadow: '0 0 0 1px #22D3EE inset' },
  commercialBanner: { marginTop: 14, border: '1px solid #0E7490', borderRadius: 12, background: '#082F49', padding: 16 },
};
...
export default function BookingWidget({ onBookingSubmit }) {
  ...
  const [commercial, setCommercial] = useState({
    ...
    notes: '',
    idempotencyKey: makeIdempotencyKey('commercial-walkthrough'),
  });
  ...
  const commercialMarketInfo = useMemo(() => MARKET_OPTIONS.find((item) => item.value === commercial.market) || MARKET_OPTIONS[0], [commercial.market]);
  ...
  function updateCommercial(name, value) {
    setCommercial((current) => ({ ...current, [name]: value }));
    setCommercialStatus('');
    setError('');
  }
  ...
  async function submitCommercialWalkthrough() {
    if (busy) return;
    setBusy(true);
    setError('');
    setCommercialStatus('');
    try {
      const response = await fetch('/api/bookings/commercial-walkthrough', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ walkthroughData: commercial }),
      });
      const result = await response.json();
      if (!response.ok || !result.success) throw new Error(result.error || 'Unable to submit commercial walkthrough request.');
      setCommercialStatus(`Walkthrough request received. Reference: ${result.serviceRequestId || 'pending'}. Our estimating team will confirm the walkthrough before preparing your custom proposal.`);
      setCommercial((current) => ({ ...current, idempotencyKey: makeIdempotencyKey('commercial-walkthrough') }));
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err));
    } finally {
      setBusy(false);
  ...
  return (
    <section style={styles.shell} data-public-booking-widget="true">
      <div style={styles.modeWrap} aria-label="Service type">
        <button type="button" style={{ ...styles.mode, ...(mode === 'residential' ? styles.modeActive : {}) }} onClick={() => switchMode('residential')}>
          Residential Cleaning
        </button>
        <button type="button" style={{ ...styles.mode, ...(mode === 'commercial' ? styles.modeActive : {}) }} onClick={() => switchMode('commercial')}>
          Commercial Cleaning
        </button>
      </div>

      {mode === 'commercial' ? (
        <div data-commercial-walkthrough="true">
          <h2 style={styles.title}>Request a Commercial Facility Walkthrough</h2>
          <p style={styles.muted}>Commercial cleaning is estimated after we understand your facility, operating requirements, cleaning frequency, access, and compliance needs.</p>
          <div style={styles.commercialBanner}>
            <strong>Custom Commercial Proposal — On-Site Facility Walkthrough Required</strong>
            <div style={{ ...styles.muted, marginTop: 6, color: '#BAE6FD' }}>No instant dollar estimate is generated. Your request goes directly to Revenue / Estimating and does not create a cleaning job.</div>
          </div>

          <div style={styles.section}>
            <h3 style={styles.title}>Company & Facility</h3>
            <div style={styles.grid}>
              <label style={styles.label}>Service market
                <select style={styles.input} value={commercial.market} onChange={(e) => updateCommercial('market', e.target.value)}>
                  {MARKET_OPTIONS.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}
                </select>
              </label>
              <label style={styles.label}>Company name<input style={styles.input} value={commercial.companyName} onChange={(e) => updateCommercial('companyName', e.target.value)} /></label>
              <label style={styles.label}>Primary contact name<input style={styles.input} value={commercial.contactName} onChange={(e) => updateCommercial('contactName', e.target.value)} /></label>
              <label style={styles.label}>Primary contact email<input style={styles.input} type="email" value={commercial.email} onChange={(e) => updateCommercial('email', e.target.value)} /></label>
              <label style={styles.label}>Primary contact phone<input style={styles.input} type="tel" value={commercial.phone} onChange={(e) => updateCommercial('phone', e.target.value)} /></label>
              <label style={styles.label}>Service address<input style={styles.input} value={commercial.address} onChange={(e) => updateCommercial('address', e.target.value)} /></label>
              <label style={styles.label}>City<input style={styles.input} value={commercial.city} onChange={(e) => updateCommercial('city', e.target.value)} /></label>
              <label style={styles.label}>{commercialMarketInfo.regionLabel}<input style={styles.input} value={commercial.postalCode} onChange={(e) => updateCommercial('postalCode', e.target.value)} placeholder={commercial.market === 'HUC-ON' ? 'A1A 1A1' : '85001'} /></label>
              <label style={styles.label}>Facility type
                <select style={styles.input} value={commercial.facilityType} onChange={(e) => updateCommercial('facilityType', e.target.value)}>
                  {FACILITY_OPTIONS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}
                </select>
              </label>
              <label style={styles.label}>Estimated square footage<input style={styles.input} type="number" min="1" step="1" value={commercial.estimatedSquareFeet} onChange={(e) => updateCommercial('estimatedSquareFeet', e.target.value)} placeholder="e.g. 12000" /></label>
              <label style={styles.label}>Preferred cleaning frequency
                <select style={styles.input} value={commercial.frequency} onChange={(e) => updateCommercial('frequency', e.target.value)}>
                  {COMMERCIAL_FREQUENCY_OPTIONS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}
                </select>
              </label>
            </div>
          </div>
          ...
          <div style={styles.section}>
            <h3 style={styles.title}>Preferred Walkthrough Date & Time Window</h3>
            <div style={styles.grid}>
              <label style={styles.label}>Preferred walkthrough date<input style={styles.input} type="date" value={commercial.walkthroughDate} onChange={(e) => updateCommercial('walkthroughDate', e.target.value)} /></label>
              <label style={styles.label}>Preferred time window
                <select style={styles.input} value={commercial.walkthroughTimeWindow} onChange={(e) => updateCommercial('walkthroughTimeWindow', e.target.value)}>
            ...
            <label style={{ ...styles.label, marginTop: 14 }}>Facility notes / access / special requirements
              <textarea style={{ ...styles.input, minHeight: 90 }} value={commercial.notes} onChange={(e) => updateCommercial('notes', e.target.value)} />
            </label>
          </div>

          <div style={styles.actions}>
            <button type="button" style={{ ...styles.confirm, ...(busy ? styles.disabled : {}) }} onClick={submitCommercialWalkthrough} disabled={busy}>
              {busy ? 'Submitting…' : 'Request Facility Walkthrough'}
            </button>
          </div>
          <p style={{ ...styles.muted, marginBottom: 0 }}>Submitting this form requests an estimating walkthrough only. A cleaning job is created only after a commercial proposal is explicitly accepted.</p>
          {commercialStatus ? <div role="status" style={styles.success}>{commercialStatus}</div> : null}
          {error ? <div role="alert" style={styles.error}>{error}</div> : null}
        </div>
      ) : (
        <div data-residential-booking="true">
          <h2 style={styles.title}>Tell us about your residential cleaning</h2>
          <div style={styles.grid}>
            <label style={styles.label}>Service market
