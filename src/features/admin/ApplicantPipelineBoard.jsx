...
export default function ApplicantPipelineBoard({ applicants, onReview }) {
  return <section className="team-panel applicant-pipeline" aria-labelledby="applicant-pipeline-title">
    ...
    <div className="applicant-pipeline__board">{HIRING_STAGES.map((stage) => {
      ...
      return <section className="applicant-stage" key={stage} aria-label={`${stage} applicants`}><header><h4>{stage}</h4><span>{items.length}</span></header><div>{items.map((applicant) => <button className="applicant-card" type="button" key={applicant.id} data-tip="Applicant profile — review background checks and advance stage." onClick={() => onReview(applicant)}><strong>{applicant.name}</strong><span>{applicant.marketCode} · {age(applicant.ageDays)}</span><span className="applicant-card__documents">{documents(applicant)}</span><small>{applicant.nextAction}</small></button>)}{!items.length ? <p className="applicant-stage__empty">No candidates</p> : null}</div></section>;
    })}</div>
  </section>;
}
