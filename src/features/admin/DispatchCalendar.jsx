...
export default function DispatchCalendar({ jobs, weekStart, onWeekChange, onAssign }) {
  ...
  return <section className="dispatch-panel dispatch-calendar" aria-labelledby="dispatch-calendar-title">
    ...
    <div className="dispatch-calendar__week">{days.map((day) => { const scheduled = jobs.filter((job) => dayKey(job.scheduledStart) === dayKey(day)); return <article key={dayKey(day)} data-tip="Scheduled operational commitments by territory."><header><span>{day.toLocaleDateString(undefined, { weekday: "short" })}</span><strong>{day.getDate()}</strong></header><div>{scheduled.length ? scheduled.map((job) => <button type="button" className="dispatch-calendar__job" key={job.id} onClick={() => onAssign(job)}><span><time>{time(job.scheduledStart)}</time><DispatchJobBadge job={job} /></span><strong>{job.customerName}</strong>{job.isCommercial ? <small>{job.squareFeet ? `${job.squareFeet.toLocaleString()} sq ft` : "Walkthrough scope"} · {job.facilityContact || "Facility contact pending"}</small> : <small>{job.workerNames.join(", ") || "Assignment pending"}</small>}<StatusBadge tone={job.operationalStatus === "in_progress" ? "warning" : "info"}>{job.operationalStatus.replaceAll("_", " ")}</StatusBadge></button>) : <p>No jobs</p>}</div></article>; })}</div>
  </section>;
}
