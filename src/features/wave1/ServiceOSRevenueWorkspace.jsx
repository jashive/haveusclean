...
const styles = {
  ...
  metric: { background: "#0C1E1B", borderRadius: 8, padding: 10 },
  metricLabel: { color: "#89A8A1", fontSize: 11, textTransform: "uppercase", fontWeight: 850 },
  metricValue: { marginTop: 4, fontWeight: 850 },
...
function packageLabel(key) {
  return PACKAGE_OPTIONS.find((item) => item.value === key)?.label || key;
}

function frequencyLabel(key) {
  return FREQUENCY_OPTIONS.find((item) => item.value === key)?.label || key;
}
...
export default function ServiceOSRevenueWorkspace({ session, revenueContext }) {
  ...
  const customerText = useMemo(() => {
    if (!quote) return "";
    return buildCustomerFacingQuoteText({
  ...
  async function handleGenerateQuote() {
    ...
    try {
      ...
      const managementReason = getManagementReviewReason({
      ...
      if (managementReason) {
        setReviewReason(managementReason);
        return;
      }

      const config = await fetchPublishedGovernedResidentialConfig({
        accessToken,
      ...
      const approvedSelections = getDefaultApprovedSelections(config, {
        condition: form.condition,
      ...
      if (rawQuote?.requiresOfficeReview) {
        setReviewReason(rawQuote.reason || "This scope requires management review.");
        return;
      }
      const completedQuote = applyGovernedResidentialAddons(rawQuote, config, form.addons);
      completedQuote.input = {
        dwellingType: form.dwellingType,
        beds: Number(form.beds),
        baths: Number(form.baths),
        ...
        condition: form.condition,
        frequency: form.frequency,
        sqftBand: form.sqftBand || null,
        addons: form.addons,
      };
      setConfigurationVersion(config);
      setQuote(completedQuote);
    } catch (err) {
      const message = err?.message || "Unable to generate quote.";
      if (/matrix row not found|office review/i.test(message)) setReviewReason(message);
  ...
  async function handleSaveDraft() {
    ...
    try {
      const requirements = {
        ...
        location: { address: form.address.trim(), city: form.city.trim(), postalCode: form.postalCode.trim() || null },
        scope: {
          dwellingType: form.dwellingType,
          sqft: form.sqft ? Number(form.sqft) : null,
          beds: Number(form.beds),
          ...
          preferredWindow: form.preferredWindow || null,
          notes: form.notes.trim() || null,
        },
      };
      const metadata = { source: "serviceos_native_quote", lead_source: form.leadSource || null };
      const title = `${form.customerName.trim()} — ${packageLabel(form.packageKey)}`;

      const serviceRequest = await createServiceRequest(buildServiceRequestPayload({
        organizationId: orgId,
        businessUnitId,
        serviceCategory: "residential",
        lifecycleStatus: "qualified",
        intakeChannel: "office_native_quote",
        title,
      ...
      const opportunity = await createOpportunity(buildOpportunityPayload({
        organizationId: orgId,
        businessUnitId,
        serviceRequestId: serviceRequest.id,
        stage: "proposal",
        title,
        summary: `Native residential quote for ${form.customerName.trim()}`,
        metadata,
        appUserId,
      }), accessToken);

      const estimate = await createEstimate(buildEstimatePayload({
        organizationId: orgId,
        businessUnitId,
        opportunityId: opportunity.id,
        lifecycleStatus: "prepared",
        versionNo: 1,
        assumptions: { pricing_authority: configurationVersion.version, pricing_mode: "published_configuration" },
        scopeSnapshot: requirements.scope,
        notes: form.notes.trim() || null,
        metadata,
        appUserId,
      }), accessToken);
      ...
      const pricingSnapshot = await createPricingSnapshot(capturePricingSnapshot({
        quote,
        organizationId: orgId,
        businessUnitId,
        opportunityId: opportunity.id,
      ...
      const quoteRecord = await createQuote(buildQuotePayload({
        organizationId: orgId,
        businessUnitId,
        opportunityId: opportunity.id,
        estimateId: estimate.id,
      ...
      const quoteVersion = await createQuoteVersion(buildQuoteVersionPayload({
        ...
        estimateId: estimate.id,
        versionNo: 1,
        title: packageLabel(form.packageKey),
        lineItemsSnapshot: [{
          service: packageLabel(form.packageKey),
          subtotal: quote.preTaxTotal,
          tax: quote.taxAmount,
          total: quote.total,
          addons: quote.addonLines || [],
        }],
        commercialSnapshot: { customerFacingText: customerText },
        metadata,
        appUserId,
      }), accessToken);
      ...
      setError(err?.message || "Unable to save canonical draft.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function handleRecordSent() {
    ...
    try {
      ...
      setError(err?.message || "Unable to record quote as sent.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <section style={styles.panel} data-testid="serviceos-native-revenue-workspace">
      <h2 style={styles.heading}>Guided Intake + Quick Quote</h2>
      <p style={styles.subheading}>Use this for ordinary Ontario residential leads. Pricing comes from the HEMS-published configuration. Customer acceptance and job creation remain separate real-world steps.</p>
      <div style={styles.steps} aria-label="Quote workflow">
        <span style={styles.step}>1 · Qualify</span><span style={styles.step}>2 · Generate Quote</span><span style={styles.step}>3 · Save Draft</span><span style={styles.step}>4 · Send + Record</span>
      </div>
      ...
      <div style={styles.section}>
        <h3 style={styles.sectionTitle}>Residential qualification</h3>
        <div style={styles.grid}>
      ...
      <div style={styles.section}>
        ...
        <div style={styles.note}>Published “starting” and “minimum” add-on prices are used for ordinary scopes. Custom specialty work routes to management review.</div>
      </div>

      <div style={styles.actions}>
        <button type="button" style={{ ...styles.primary, ...(busy ? styles.disabled : {}) }} disabled={busy} onClick={handleGenerateQuote}>{busy ? "Working…" : "Generate Quote"}</button>
      </div>
      ...
      {quote ? (
        <div style={styles.result} data-testid="serviceos-native-quote-result">
          <div style={styles.label}>Recommended customer total</div>
          <div style={styles.money}>CA${Number(quote.total || 0).toFixed(2)}</div>
          <div style={styles.status}>READY TO QUOTE</div>
          <div style={styles.resultGrid}>
            <div style={styles.metric}><div style={styles.metricLabel}>Pre-tax</div><div style={styles.metricValue}>${Number(quote.preTaxTotal || 0).toFixed(2)}</div></div>
            <div style={styles.metric}><div style={styles.metricLabel}>HST</div><div style={styles.metricValue}>${Number(quote.taxAmount || 0).toFixed(2)}</div></div>
            ...
            <div style={styles.metric}><div style={styles.metricLabel}>Add-ons</div><div style={styles.metricValue}>${Number(quote.addonTotal || 0).toFixed(2)}</div></div>
          </div>
          ...
          {saved ? (
            <div style={styles.section}>
              <h3 style={styles.sectionTitle}>Canonical quote record</h3>
              <div style={styles.note}>Service Request: {saved.serviceRequest?.id}<br />Quote Version: {saved.quoteVersion?.id}</div>
              <div style={styles.actions}>
                <button type="button" style={{ ...styles.primary, ...(busy || saved.sent ? styles.disabled : {}) }} disabled={busy || saved.sent} onClick={handleRecordSent}>{saved.sent ? "Quote Recorded as Sent" : "I Sent This Quote — Record Sent"}</button>
              </div>
