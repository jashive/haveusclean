...
function config() {
  ...
  return { url, anon, tenantId, clientId, clientSecret, senderEmail };
}
...
export async function handleCustomerCompletionReceipt(req, res, bearer) {
  ...
  if (context.delivery_status === 'sent') return res.status(200).json({ success: true, alreadySent: true, workOrderId });
  const cfg = config();
  try {
    ...
    return res.status(200).json({ success: true, alreadySent: false, workOrderId });
  } catch (error) {
