...
test('acceptance explicit configuration resolves approved project',()=>{const c=getSupabaseConfig({VITE_SERVICEOS_ENVIRONMENT:'acceptance',VITE_SUPABASE_URL:`https://${ACCEPTANCE_SUPABASE_PROJECT_REF}.supabase.co`,VITE_SUPABASE_ANON:key});assert.equal(c.projectRef,ACCEPTANCE_SUPABASE_PROJECT_REF);assert.equal(c.isAcceptance,true)});
...
test('preview can never target production',()=>assert.throws(()=>getSupabaseConfig({VERCEL_ENV:'preview',VITE_SUPABASE_URL:`https://${PRODUCTION_SUPABASE_PROJECT_REF}.supabase.co`,VITE_SUPABASE_ANON:key}),/forbidden/));
test('production requires explicit approval',()=>{const base={VERCEL_ENV:'production',VITE_SUPABASE_URL:`https://${PRODUCTION_SUPABASE_PROJECT_REF}.supabase.co`,VITE_SUPABASE_ANON:key};assert.throws(()=>getSupabaseConfig(base),/forbidden/);assert.equal(getSupabaseConfig({...base,VITE_SERVICEOS_PRODUCTION_APPROVED:'true'}).projectRef,PRODUCTION_SUPABASE_PROJECT_REF)});
